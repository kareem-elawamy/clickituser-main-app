...
class _AsterThemeHomeScreenState extends State<AsterThemeHomeScreen> {
  ...
  Future<void> _loadData(bool reload) async {
    ...
    ]); // intentionally NOT awaited — loads in background

    // Group 4: Auth-gated data
    if (Provider.of<AuthController>(Get.context!, listen: false).isLoggedIn()) {
      await Future.wait([
        Provider.of<ProfileController>(Get.context!, listen: false).getUserInfo(Get.context!),
        Provider.of<SellerProductController>(Get.context!, listen: false).getShopAgainFromRecentStore(),
        Provider.of<OrderController>(Get.context!, listen: false).getOrderList(1, 'delivered', type: 'reorder'),
  ...
  void initState() {
    ...
    _scrollController.addListener(() {
      if (_scrollController.position.pixels >= _scrollController.position.maxScrollExtent * 0.8) {
        if (!Provider.of<ProductController>(context, listen: false).filterIsLoading) {
          Provider.of<ProductController>(context, listen: false).getLatestProductList(
            Provider.of<ProductController>(context, listen: false).lOffset + 1
          );
        }
      }
    });
      ...
      body: SafeArea(child: RefreshIndicator(
          ...
          child: CustomScrollView(controller: _scrollController, slivers: [
            SliverAppBar(
              ...
              title: Image.asset(BrandConfig.appLogo, height: 35), actions: const [
              CartHomePageWidget()]),
            ...
            SliverToBoxAdapter(
              child: singleVendor? const SizedBox():
            ...
            SliverToBoxAdapter(
              child: SizedBox( child: Consumer<ProductController>(
                  builder: (context, productController,_) {
