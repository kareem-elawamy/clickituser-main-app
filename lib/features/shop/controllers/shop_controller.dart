...
import 'package:flutter_sixvalley_ecommerce/features/shop/domain/models/seller_info_model.dart' as seller_info;
...
class ShopController extends ChangeNotifier {
  ...
  seller_info.SellerInfoModel? sellerInfoModel;
  Future<void> getSellerInfo(String sellerId) async {
    ...
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      sellerInfoModel = seller_info.SellerInfoModel.fromJson(apiResponse.response!.data);
    } else {
  ...
  void initTopSellerData(dynamic data) {
    if (data is List) {
      // Aggregated API returns a bare List of seller objects
      sellerModel = SellerModel(
        totalSize: data.length,
        limit: data.length,
        offset: 0,
        sellers: data.map<Seller>((v) => Seller.fromJson(v as Map<String, dynamic>)).toList(),
      );
    } else if (data is Map) {
      sellerModel = SellerModel.fromJson(Map<String, dynamic>.from(data));
    }
    notifyListeners();
  }
  ...
  Future<void> getTopSellerList(bool reload, int offset, {required String type}) async {
      ...
      if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
        if(offset == 1){
          initTopSellerData(apiResponse.response?.data);
        }else{
