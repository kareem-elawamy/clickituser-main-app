...
// enum Address {shipping, billing }
  ...
  void _checkPermission(Function callback, BuildContext context) async {
    ...
    // تعديل هام: whileInUse تعني أن المستخدم "وافق"، لذا لا يجب وضعها مع حالة الرفض
    if (permission == LocationPermission.denied) {
      if (context.mounted) {
        showDialog(
          context: context,
          builder: (context) => AlertDialog(
            content: SuccessDialog(
              ...
              title: '',
              description:
                  getTranslated('you_denied', context) ?? 'Permission denied',
            ),
            actions: [
              TextButton(
          ...
          builder: (context) => AlertDialog(
            content: SuccessDialog(
              ...
              description: getTranslated('you_denied', context) ??
                  'Permission denied forever. Please enable from settings.',
            ),
            actions: [
              TextButton(
                onPressed: () async {
      ...
      // هذه الحالة تعمل إذا كانت الصلاحية whileInUse أو always (أي تمت الموافقة)
      callback();
    }
  }
}
